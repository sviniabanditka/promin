...
func TestParseXMLTVMatchesByNameAndAltName(t *testing.T) {
	...
	progs, matched, err := parseXMLTV(strings.NewReader(xmltvFixture), ours, w)
	if err != nil {
		t.Fatal(err)
	}
	if matched["1Plus1Ukraina.ua"] != "1-1-ukraina" || matched["1Plus1.ua"] != "1-1" || matched["24Kanal.ua"] != "24kanal" {
		t.Fatalf("bad match: %v", matched)
	}
	if _, ok := matched["Nope.ua"]; ok {
		t.Fatal("Nope must not match")
	}
	if len(progs) != 2 { // the 2020 programme is outside the window
		t.Fatalf("want 2 programmes, got %d: %+v", len(progs), progs)
	}
